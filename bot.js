...
export default async function handler(req, res) {
  ...
  if (state) {
    ...
    if (state.step === 'task_reward') {
      state.reward = parseInt(text);
      if (!state.reward || isNaN(state.reward)) { await sendMessage(chatId, '❌ Invalid reward. Enter a number:'); return res.status(200).json({ ok: true }); }
      state.step = 'task_quantity';
      await sendMessage(chatId, `📋 <b>Step 4/5</b>\n\nHow many users can complete this task?\n(Enter number, or <code>0</code> for unlimited):`);
      return res.status(200).json({ ok: true });
    }
    if (state.step === 'task_quantity') {
      state.maxCompletions = parseInt(text) || 0;
      state.step = 'task_type';
      await sendMessage(chatId,
        `📋 <b>Step 5/5</b>\n\nIs this a <b>Telegram API task</b>?\n(channel/group join — bot must be admin)\n\nor a <b>regular task</b>? (YouTube, Facebook etc.)`,
        { reply_markup: { inline_keyboard: [
          [{ text: '✅ Yes — Telegram API', callback_data: 'task_type_api' }],
          [{ text: '❌ No — Regular task', callback_data: 'task_type_none' }],
        ]}}
      );
      return res.status(200).json({ ok: true });
    }
    if (state.step === 'task_confirm' || state.step === 'task_confirm_api') {
      if (text.toUpperCase() !== 'CONFIRM') { await sendMessage(chatId, '❌ Type CONFIRM to save, or use ◀️ Cancel.'); return res.status(200).json({ ok: true }); }
      const taskId = 'task_' + Date.now();
      await tasks.insertOne({
        id: taskId, title: state.title, link: state.link,
        reward: state.reward, maxCompletions: state.maxCompletions,
        type: state.type, active: true, createdAt: new Date(), completedCount: 0,
      });
      await sendMessage(chatId,
        `✅ <b>Task created!</b>\n\n📋 ${state.title}\n💰 ${state.reward} EG\n🔗 ${state.link || 'No link'}\n👥 Max: ${state.maxCompletions || 'Unlimited'}\n🔧 Type: ${state.type}`,
        { reply_markup: adminMenu }
      );
      delete taskState[fromId];
      return res.status(200).json({ ok: true });
  ...
  // Fallback: show admin menu
  if (fromId === String(ADMIN_ID)) {
    await sendPhoto(chatId, START_PHOTO, '👑 <b>Ton Edge Admin Panel</b>', { reply_markup: adminMenu });
  }
