...
export default async function handler(req, res) {
  ...
  if (update.callback_query) {
    ...
    if (data === 'a_addpromo') {
      state[fromId] = { step: 'promo_code' };
      await edit(chatId, msgId, '🎟 Enter <b>promo code</b> (e.g. LAUNCH2026):', { reply_markup: { inline_keyboard: [[{ text: '◀️ Cancel', callback_data: 'a_menu' }]] } });
      return res.status(200).json({ ok: true });
    }
  ...
  const s = state[fromId];
  if (!s) {
    await sendPhoto(chatId, START_PHOTO, '👑 <b>Ton Edge Admin Panel</b>', { reply_markup: adminKb });
    return res.status(200).json({ ok: true });
  }
  ...
  // Promo
  if (s.step === 'promo_code') { s.code = text.toUpperCase().trim(); s.step = 'promo_reward'; await send(chatId, `🎟 Code: <b>${s.code}</b>\n\nEnter <b>EG reward</b>:`); return res.status(200).json({ ok: true }); }
  if (s.step === 'promo_reward') { s.reward = parseInt(text); s.step = 'promo_maxuses'; await send(chatId, `Reward: <b>${s.reward} EG</b>\n\nEnter <b>max uses</b> (0 = unlimited):`); return res.status(200).json({ ok: true }); }
  if (s.step === 'promo_maxuses') {
    const maxUses = parseInt(text) || 9999;
    delete state[fromId];
    await promos.updateOne({ code: s.code }, { $set: { code: s.code, reward: s.reward, maxUses, usedCount: 0, createdAt: new Date() } }, { upsert: true });
    await send(chatId, `✅ Promo <b>${s.code}</b> created!\nReward: <b>${s.reward} EG</b> | Max uses: <b>${maxUses}</b>`, { reply_markup: adminKb });
    return res.status(200).json({ ok: true });
  }
